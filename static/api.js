// 前后端由同一个 Worker 托管，默认使用同源 API；本地调试时仍可覆盖。
const API_BASE_URL = (window.WEBURL_API_BASE_URL || '/api').replace(/\/$/, '');
let token = null;
...
function setToken(newToken) {
    ...
    if (token) {
        ...
        localStorage.removeItem('admin_token'); // 清理旧版本遗留的长期令牌
    }
}
...
async function saveBackgroundSettingsAPI(data) {
    return fetchAPI('/background', {
        method: 'PUT',
        body: JSON.stringify(data)
    });
...
async function deleteBackgroundSettings() {
    return fetchAPI('/background', {
        method: 'DELETE'
    });
}
